export const mergeOptionsWithDefaults = (options: Options): MergedOptions => ({
    ...defaultOptions,
    ...options
})
const getAccessToken = async (options: Options): Promise<string> => {
    ...
    const stsToken: string = await fetch(mergedOptions.stsEndpoint, {
        method: 'POST',
        headers: {
    ...
    }).then(async res => await res.json()).then(res => res.access_token)
    ...
    }).then(async res => await res.json()).then(res => res.token)
}
